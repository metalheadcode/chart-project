...
import { getStocksByMarketRequest } from '../../redux/slicers/stocks';
import { StockState, StockItem } from "types/redux";
...
function MainScreen() {
  ...
  const { data, marketStr, loading } = useSelector((state: {
    stocks: StockState
  }) => state.stocks);
  ...
  // STATES
  const [chartData, setChartData] = useState<StockItem[]>([]);
  ...
  // FUNCTIONS
  const getStocksByMarket = (m: string) => {
    dispatch(getStocksByMarketRequest({
      market: m,
      onCallback: (response: { status: number, error?: string }) => {
  ...
  }, [data, marketStr, getStocksByMarket]);
